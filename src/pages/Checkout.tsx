...
const Checkout = () => {
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      try {
        const { preferenceId: prefId } = await paymentService.createPreference({
          pedidoId: order.id,
          metodo: "pix",
        });
  ...
  const handlePaymentSuccess = (_pedidoId: string) => {
    clearCart();
    navigate("/meus-pedidos");
  };

  const handlePaymentPending = (_pedidoId: string) => {
    clearCart();
    navigate("/meus-pedidos");
  };
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        <div className="container py-8">
          ...
          <h1 className="text-2xl font-extrabold md:text-3xl mb-6">
            {step === "cart" ? "Resumo do Pedido" : "Pagamento"}
          </h1>
            ...
            <div className="max-w-2xl mx-auto">
              {!publicKey ? (
                <div className="rounded-xl border border-destructive bg-destructive/10 p-6 text-center text-destructive">
                  Pagamento indisponível no momento. Chave pública não configurada.
                </div>
              ) : (
                <PaymentBrick
